...
# With slight assistance from Sairam Suresh from S401
# Imports
...
def brightsparks():
    ...
    def websitescraper(link):
        ...
        possible_hrefs = ['//a[@href="scholarship.php"]', '//a[@href="sçcholarships.php"]', '//a[@href="scholar.php"]',
                          '//a[@href="mpa-scholarship-programme.php"]', '//a[@href="ss-scholarship.php"]',
                          '//a[@href="uni.php"]',
                          '//a[@href="midterm.php?schid=10441"]', '//area[@href="scholarship.php"]',
                          '//a[@href="phd.php"]',
                          '//a[@href="scholarships_bcaundergrad.php"]',
                          '//a[@href="csit-undergraduate-scholarship.php"]',
        ...
        try:
            ...
            print([a.get_attribute("href") for a in drive.find_elements(By.CSS_SELECTOR, "a") if
                   (("view" in a.get_attribute("href") or "php" in a.get_attribute("href")) and "dukenus" in save) or (
                               "view" in (a.get_attribute("href") and "php" in a.get_attribute("href")))])
            if [a.get_attribute("href") for a in drive.find_elements(By.CSS_SELECTOR, "a") if
                ...
                for i in [a.get_attribute("href") for a in drive.find_elements(By.CSS_SELECTOR, "a") if
                    ...
                    paired_links.append([openlink.url, re.sub(r"\n+", "\n", "".join(
                        [i.text for i in opensoup.find("div", class_="scholarship_description")] if opensoup.find("div",
                                                                                                                  class_="scholarship_description") is not None and "dukenus" in save else [
                            i.text for i in opensoup.find_all("p")]))])
        ...
        time.sleep(5 + random.randrange(0, 4))
        soup = BeautifulSoup(drive.page_source, 'html.parser')
        save = drive.current_url
        print(save)
        drive.quit()
        ...
        linklist = [a["href"] for a in soup.find_all('a', href=True) if
                    "http" in a["href"] and "brightsparks" in a["href"] and a["href"] != "https://brightsparks.com.sg/"]
        ...
        if not linklist:
            ...
            try:
                for i in range(len(linklist)):
                    ...
                    opensoup = BeautifulSoup(openlink.content, 'html.parser')
                    paired_links.append([openlink.url, re.sub(r"\n+", "\n", "".join(
                        [i.getText() for i in opensoup.find("div", id="scholarship_description")] if opensoup.find(
                            "div", id="collapseOne") is None else [i.getText() for i in opensoup.find("div",
                                                                                                      id="collapseOne")] if opensoup.find(
                            "div", id="wrapper") is None else [i.getText() for i in opensoup.find("div",
                                                                                                  id="scholarship_description")] if opensoup.find(
                            "div", id="collapseOne") is None else [i.getText() for i in
                                                                   opensoup.find("div", id="collapseOne")]))])
            except:
                return ""
        return paired_links
    ...
    sleep(1)
    information = []
    for title in driver.find_elements(By.CLASS_NAME, 'col-span-11'):
        elem = title.find_element(By.TAG_NAME, "a")
        site = elem.get_attribute('href').split("(")[0]
        if site not in sitelist and "jtc" not in site.lower():
